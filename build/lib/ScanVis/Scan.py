...
class Scan:
  ...
  def normalise_color(self):
    for key in ['scan', 'shifted_seg', 'seg', 'brain']:
  ...
  def compare_buttloads_of_slices(self, other : Scan, image = 'scan', slices = 5, structure_id = None, ms = 2, c = 'w', ax = None, figsize = (3,3), dpi = 100, pad = -2, save = None, title = None):    
    ax, ax_exists, fig = self.check_ax(ax, slices, 6, [figsize[0]*slices, figsize[1]*6], dpi, pad)
    for i in [0,2,4]: ax[i] = self.plot_hella_slices(image, int(i/2), slices, structure_id, ms, c, ax[i], figsize, dpi, pad, None, False, plot_legend = i == 0)
    for i in [1,3,5]: ax[i] = other.plot_hella_slices(image, int((i-1)/2), slices, structure_id, ms, c, ax[i], figsize, dpi, pad, None, False, plot_legend = False)
