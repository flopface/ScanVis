...
class Data(Scant):
  def __init__(self, array, parent : Scan, average = 1):
    ...
    self.arrays['brain'] = self.array * parent.arrays['mask']
    if self.average != None: self.arrays['brain'] += self.average*(~(parent.arrays['mask'].astype(bool))).astype(int)
  ...
  def normalise_color(self):
    ...
    for key in self.arrays: 
      ...
      if key in ['scan', 'brain'] and self.average != None:
        if biggest > -smallest: smallest = (2*self.average)-biggest
        else: biggest = (2*self.average)-smallest
